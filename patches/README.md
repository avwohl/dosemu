...
These are **toolchain** patches. They are not applied by the dosiz build:
dosiz runs on the in-tree emu88 386 core and has no build-time dependencies.
They are kept here because the docs under `docs/` reference them and because
anyone rebuilding the same toolchains will hit the same problems.

| File | For |
|---|---|
| `djgpp-libc-c1loadef-stack-smash.patch` | DJGPP libc 2.05 -- stack-buffer overflow in `__crt0_load_environment_file` |
| `verify-c1loadef-patch.sh` | reproduces the crash and proves the patch fixes it |
| `watcom-wlink.lnk` | Open Watcom v2 -- restores the `system dos4g` linker definition |
...
Not a patch but a configuration file. Open Watcom v2's `binw/wlink.lnk`
no longer carries the `system dos4g` definition that the 11.0c release had,
so linking a DOS/4GW target fails at run time with:

    DOS/4GW fatal error (1012): FOO.EXE is not a WATCOM program

Drop this file into `<watcom>/binw/wlink.lnk`. See
[`../docs/watcom-setup.md`](../docs/watcom-setup.md) for the full setup.

## Submission status

This section concerns the DJGPP libc patch only.
